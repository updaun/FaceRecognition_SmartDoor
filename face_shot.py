    ...
    if name == '':
        ...
        def createFolder(directory):
            try:
                ...
                messagebox.showinfo("스마트 도어 시스템", "[오류] 폴더 생성을 실패하였습니다.")

        createFolder('./dataset/'+name)
        print("[ 카메라를 준비하고 있습니다. ]\n")
        print(
            '[알림] 지금부터 체험자의 얼굴을 100장 촬영합니다.\n[알림] 카메라를 응시해 주세요.\n[ SPACE ] 를 누르면 촬영을 중단할 수 있습니다. \n')
        messagebox.showinfo(
            "스마트 도어 시스템", "[ 확인 ] 버튼을 누르면 사진 촬영을 시작합니다. \n \n카메라를 응시해야만 촬영을 진행합니다.")
        messagebox.showinfo("스마트 도어 시스템", "[ SPACE ] 를 누르면 촬영을 중단할 수 있습니다.")
        ...
        while True:
            ...
            if face_extractor(frame) is not None:
                ...
                draw.text(
                    (50, 50),  '[ 진행 정도 : ' + str(count)+'% ]', font=font, fill=(b, g, r, a))
                img = np.array(img_pil)
                cv2.imshow('Press   SPACE   key to exit', img)
                cv2.moveWindow("Press   SPACE   key to exit", 470, 0)
            else:
            ...
            if k % 256 == 27 or k % 256 == 13 or k % 256 == 32 or count == 100:
                break
